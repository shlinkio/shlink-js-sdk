...
export class ShlinkApiClient implements BaseShlinkApiClient {
  ...
  async #performVisitsRequest(options: ShlinkRequestOptions): Promise<ShlinkVisitsList> {
  ...
  async #performRequest<T>(requestOptions: ShlinkRequestOptions): Promise<T> {
    return this.#httpClient.jsonRequest<T>(...this.#toFetchParams(requestOptions));
  }

  async #performEmptyRequest(requestOptions: ShlinkRequestOptions): Promise<void> {
    return this.#httpClient.emptyRequest(...this.#toFetchParams(requestOptions));
  }

  #toFetchParams({ url, method = 'GET', query, body, domain, signal }: ShlinkRequestOptions): [string, RequestOptions] {
